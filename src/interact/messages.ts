...
export class Messages {
    ...
    static combat(Case, actor, enemy) {
        ...
        let str_damageIndicator = Utils.span(`(${enemy.health + actor.weapon.weaponDamage()}=>${enemy.health})`, colors.damageGreen);
        let str_damageIndicatorCrit = Utils.span(`(${enemy.health + actor.weapon.weaponDamage() * 2}=>${enemy.health})`, colors.damageGreen);
        let str_damageIndicatorCrit0 = Utils.span(`(${enemy.health + actor.weapon.weaponDamage() * 2}=>0)`, colors.damageGreen);
        ...
        switch (Case) {
            ///ACTOR MESSAGES///
            case 'hitNormal':
                Utils.printLine(`${playerName} ${damageType} ${targetName} with ${playerWeapon} and caused ${str_actorDamage} damage. ${enemyHealth}`);
                break;
            case 'hitCritical':
                Utils.printLine(`${str_Critical} ${playerName} ${damageType} ${targetName} with ${playerWeapon} and caused ${str_actorDamage} damage. ${enemyHealthCrit}`);
                break;
            case 'hitCriticalKill':
                Utils.printLine(`${str_Critical} ${playerName} ${damageType} ${targetName} with ${playerWeapon} and caused ${str_actorDamageCrit} damage. ${str_damageIndicatorCrit0}`);
                break;
            case 'kill':
            ...
            case 'hitMiss':
                switch (randomcase(3)) {
                    case 0:
                        Utils.printLine(`${hitMiss} ${playerName} tried to attack ${targetName} but ${pronounS} was able to dodge the ${damageType}!`);
                        break;
                    case 1:
                        Utils.printLine(`${hitMiss} ${playerName} tried to attack ${targetName} but missed!`);
                        break;
                    case 2:
                        Utils.printLine(`${hitMiss} ${targetName} was able to jump away from ${playerName}'s ${damageType}!`);
                        break;
                }
                break;
            case 'encounter':
                Utils.printLine(`You encountered ${targetName}. Just by looking at ${pronounP} attire you can see ${pronounS} is a ${str_actorRole(enemy)}.`);
                break;
            case 'encounterSame':
                Utils.printLine(`You encountered ${targetName}. However you see ${pronounP} is also ${str_actorRole(enemy)} so you just greet ${pronounO} and venture forward.`);
                break;
            ///Level up message
            case 'levelUp':
            ...
            case 'loot':
                switch (randomcase(3)) {
                    case 0:
                        Utils.printLine(`As the blood still flows from ${targetName}'s liquidated carcass, you search through ${pronounP} belongings.`);
                        break;
                    case 1:
                        Utils.printLine(`You search through ${targetName}'s belongings.`);
                        break;
                    case 2:
                        Utils.printLine(`As you advance towards ${targetName}'s dead body you look around if ${pronounS} has anything valuable with ${pronounO}.`);
                        break;
                }
                break;
            case 'lootFind':
