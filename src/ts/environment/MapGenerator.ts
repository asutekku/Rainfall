...
export class MapGenerator {
    ...
    constructor(mapName: string, numberOfBuildings: number, height: number, width: number) {
        ...
        for (let i = 0; i < numberOfBuildings; i++) {
            const building = new Building("Building", width / 20, height / 20, Utils.getRandomInt(1, 40));
            building.x = (Math.round(Utils.getRandomInt(0, 20)) - 10) * width / 20;
            building.y = (Math.round(Utils.getRandomInt(0, 20)) - 10) * height / 20;
            let placeable = true;
